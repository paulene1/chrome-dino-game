...
window.onload = function () {
    ...
    gameOverImg.src = "./game-over1.png"

    //replay button
    replayImg = new Image();
    replayImg.src = "./reset.png"
...
//replay button
document.addEventListener("mousedown", checkReplayButton)
}
...
function update() {
    ...
    if (gameOver){
        context.drawImage(gameOverImg, gameOverText.x, gameOverText.y, gameOverText.width, gameOverText.height);
        context.drawImage(replayImg, replay.x, replay.y, replay.width, replay.height);
        return
    ...
    if(score >= 1000*x){
        velocityX -= 1;
        x++;
        console.log('velocity has increased to ' + velocityX + ' and x is' + x)
    }
...
const checkReplayButton = (e) => {
    const rect = board.getBoundingClientRect(); 
    const clickX = e.clientX - rect.left;
    const clickY = e.clientY - rect.top

    //check if click was inside replay button boundaries
    if (
        clickX >= replay.x &&
        clickX <= replay.x + replay.width &&
        clickY >= replay.y &&
        clickY <= replay.y + replay.height
    ){
        resetGame();
